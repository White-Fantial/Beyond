...
export async function PATCH(
  ...
  try {
    ...
    const option = await updateBackofficeModifierOption(
      storeId,
      tenantId,
      ctx.userId,
      optionId,
    ...
    const status =
      ...
      500;
    console.error("[backoffice/modifiers/options PATCH]", err);
...
export async function DELETE(
  ...
  try {
    ...
    const tenantId = await getStoreTenantId(storeId);
    await deleteBackofficeModifierOption(storeId, tenantId, ctx.userId, optionId);
    ...
    const status =
      ...
      message.includes("does not belong") ? 404 :
      500;
    console.error("[backoffice/modifiers/options DELETE]", err);
    return NextResponse.json({ error: message }, { status });
  }
}
