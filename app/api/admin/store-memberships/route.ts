...
import { createStoreMembership } from "@/services/admin/admin-membership.service";

export async function POST(req: NextRequest) {
  try {
    const ctx = await requirePlatformAdminNotImpersonating();
    const body = await req.json();
    const { membershipId, storeId, role, status } = body;
    const result = await createStoreMembership(
      { membershipId, storeId, role, status },
      ctx.userId
    );
