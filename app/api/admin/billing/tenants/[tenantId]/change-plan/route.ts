...
import { changeTenantPlan } from "@/services/admin/admin-subscription.service";
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    const { subscriptionId, newPlanId, note } = body;
    if (!subscriptionId || !newPlanId) {
      return NextResponse.json({ error: "subscriptionId and newPlanId are required" }, { status: 400 });
    }
    await changeTenantPlan({ tenantId, subscriptionId, newPlanId, note, actorUserId: ctx.userId });
