...
export async function GET(_req: Request, { params }: Params) {
  ...
  try {
    const ingredient = await getIngredient(tenantId, params.ingredientId);
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  try {
    const ingredient = await updateIngredient(tenantId, params.ingredientId, body);
    return NextResponse.json({ data: ingredient });
  } catch (err) {
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  const tenantId = ctx.tenantMemberships[0]?.tenantId ?? "";
  try {
    await deleteIngredient(tenantId, params.ingredientId);
