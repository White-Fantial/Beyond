import Link from "next/link";
import { requirePermission } from "@/lib/auth/permissions";
import { PERMISSIONS } from "@/lib/auth/constants";

interface Props {
  params: Promise<{ tenantSlug: string }>;
}

export default async function BrandedCustomerHomePage({ params }: Props) {
  const ctx = await requirePermission(PERMISSIONS.CUSTOMER_APP);
  const { tenantSlug } = await params;

  const base = `/${tenantSlug}/app`;

  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 mb-2">
        안녕하세요, {ctx.name} 👋
      </h1>
      <p className="text-gray-500 mb-6">주문 내역과 구독을 확인하세요.</p>

      <div className="grid grid-cols-2 gap-4">
        <Link
          href={`${base}/orders`}
          className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition"
        >
          <div className="text-2xl mb-2">📦</div>
          <div className="font-medium text-gray-900">내 주문</div>
          <div className="text-sm text-gray-500">주문 현황 조회</div>
        </Link>

        <Link
          href={`${base}/subscriptions`}
          className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition"
        >
          <div className="text-2xl mb-2">🔄</div>
          <div className="font-medium text-gray-900">구독 관리</div>
          <div className="text-sm text-gray-500">구독 현황 및 변경</div>
        </Link>

        <Link
          href={`${base}/loyalty`}
          className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition"
        >
          <div className="text-2xl mb-2">🏅</div>
          <div className="font-medium text-gray-900">리워드</div>
          <div className="text-sm text-gray-500">적립 포인트 및 혜택</div>
        </Link>

        <Link
          href={`${base}/account`}
          className="bg-white rounded-xl border border-gray-200 p-4 hover:shadow-sm transition"
        >
          <div className="text-2xl mb-2">👤</div>
          <div className="font-medium text-gray-900">내 계정</div>
          <div className="text-sm text-gray-500">계정 설정</div>
        </Link>
      </div>
    </div>
  );
}
