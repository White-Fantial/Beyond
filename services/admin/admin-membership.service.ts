...
export async function createTenantMembership(
  ...
  if (!userId?.trim()) throw new Error("User ID는 필수입니다.");
  if (!ALLOWED_MEMBERSHIP_ROLES.includes(role as AllowedMembershipRole)) {
    throw new Error(`올바르지 않은 멤버십 Role입니다: ${role}`);
  }
  ...
  if (!user) throw new Error("User를 not found.");

  const existing = await prisma.membership.findUnique({
    where: { tenantId_userId: { tenantId, userId } },
    select: { id: true },
  });
  if (existing) throw new Error("해당 User는 이미 이 Tenant에 멤버십이 있습니다.");
...
export async function updateTenantMembership(
  ...
  if (input.role !== undefined) {
    if (!ALLOWED_MEMBERSHIP_ROLES.includes(input.role as AllowedMembershipRole)) {
      throw new Error(`올바르지 않은 멤버십 Role입니다: ${input.role}`);
    }
...
export async function createStoreMembership(
  ...
  if (!storeId?.trim()) throw new Error("Store ID는 필수입니다.");
  if (!ALLOWED_STORE_ROLES.includes(role as AllowedStoreRole)) {
    throw new Error(`올바르지 않은 Store Role입니다: ${role}`);
  }
  ...
  if (!membership) throw new Error("Tenant 멤버십을 not found.");
  if (!store) throw new Error("Store을 not found.");

  // Validate cross-tenant integrity
  if (membership.tenantId !== store.tenantId) {
    throw new Error("Store 멤버십 생성 실패: 멤버십과 Store이 같은 Tenant에 속해야 합니다.");
  }
  ...
  if (existing) throw new Error("해당 멤버십은 이미 이 Store에 연결되어 있습니다.");
...
export async function updateStoreMembership(
  ...
  if (input.role !== undefined) {
    if (!ALLOWED_STORE_ROLES.includes(input.role as AllowedStoreRole)) {
      throw new Error(`올바르지 않은 Store Role입니다: ${input.role}`);
    }
    data.role = input.role;
  }
  if (input.status !== undefined) {
