...
describe("GET /api/catalog/conflicts/[conflictId]", () => {
  it("returns 404 when not found", async () => {
    ...
    const res = await getOneRoute(makeReq("GET", "http://localhost/api/catalog/conflicts/missing") as never, { params: Promise.resolve({ conflictId: "missing" }) });
  ...
  it("returns conflict details", async () => {
    ...
    const res = await getOneRoute(makeReq("GET", "http://localhost/api/catalog/conflicts/conf-1") as never, { params: Promise.resolve({ conflictId: "conf-1" }) });
    const json = await res.json();
...
describe("POST /api/catalog/conflicts/[conflictId]/start-review", () => {
  it("calls setConflictStatus with IN_REVIEW", async () => {
    ...
    const res = await startReviewRoute(makeReq("POST", "http://localhost/api/catalog/conflicts/conf-1/start-review") as never, { params: Promise.resolve({ conflictId: "conf-1" }) });
    expect(res.status).toBe(200);
...
describe("POST /api/catalog/conflicts/[conflictId]/ignore", () => {
  it("calls setConflictStatus with IGNORED", async () => {
    (mockSetStatus as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
    const res = await ignoreRoute(makeReq("POST", "http://localhost/api/catalog/conflicts/conf-1/ignore") as never, { params: Promise.resolve({ conflictId: "conf-1" }) });
    expect(res.status).toBe(200);
    const callArgs = (mockSetStatus as ReturnType<typeof vi.fn>).mock.calls[0][0];
...
describe("POST /api/catalog/conflicts/[conflictId]/resolve", () => {
  it("returns 400 when resolutionStrategy missing", async () => {
    const res = await resolveRoute(makeReq("POST", "http://localhost/api/catalog/conflicts/conf-1/resolve", {}) as never, { params: Promise.resolve({ conflictId: "conf-1" }) });
    expect(res.status).toBe(400);
  });

  it("returns 400 when resolutionStrategy invalid", async () => {
    const res = await resolveRoute(makeReq("POST", "http://localhost/api/catalog/conflicts/conf-1/resolve", { resolutionStrategy: "INVALID" }) as never, { params: Promise.resolve({ conflictId: "conf-1" }) });
    expect(res.status).toBe(400);
  });

  it("resolves with KEEP_INTERNAL strategy", async () => {
    (mockResolve as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
    const res = await resolveRoute(makeReq("POST", "http://localhost/api/catalog/conflicts/conf-1/resolve", { resolutionStrategy: "KEEP_INTERNAL", note: "Trust internal" }) as never, { params: Promise.resolve({ conflictId: "conf-1" }) });
    expect(res.status).toBe(200);
  ...
  it("resolves with ACCEPT_EXTERNAL strategy", async () => {
    (mockResolve as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
    const res = await resolveRoute(makeReq("POST", "http://localhost/api/catalog/conflicts/conf-1/resolve", { resolutionStrategy: "ACCEPT_EXTERNAL" }) as never, { params: Promise.resolve({ conflictId: "conf-1" }) });
    expect(res.status).toBe(200);
    const callArgs = (mockResolve as ReturnType<typeof vi.fn>).mock.calls[0][0];
  ...
  it("returns 400 when service throws", async () => {
    ...
    const res = await resolveRoute(makeReq("POST", "http://localhost/api/catalog/conflicts/conf-1/resolve", { resolutionStrategy: "DEFER" }) as never, { params: Promise.resolve({ conflictId: "conf-1" }) });
    expect(res.status).toBe(400);
  });
});
