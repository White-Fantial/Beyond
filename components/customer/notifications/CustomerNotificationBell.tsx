...
export default function CustomerNotificationBell({ initialUnreadCount = 0 }: { initialUnreadCount?: number }) {
  ...
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    fetch("/api/customer/notifications?pageSize=5")
      .then((r) => r.json())
      .then((body) => {
          ...
          <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-xl border border-gray-200 shadow-lg z-20 overflow-hidden">
            ...
            <div className="max-h-72 overflow-y-auto divide-y divide-gray-50">
              {loading ? (
                ...
                notifications.map((n) => (
                  <div key={n.id} className={`px-4 py-3 ${!n.readAt ? "bg-brand-50" : ""}`}>
