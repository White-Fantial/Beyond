...
- **Two-layer Owner Navigation** — Owner Portal navigation is split into (1) a global owner sidebar (always visible across all owner pages) and (2) a store-scoped secondary navigation (appended to the sidebar when inside a specific store). Store detail pages use sidebar-based navigation grouped into Store / Catalog / Operations sections; wide horizontal tab bars are not used. This structure scales as more store tools are added without overflowing the viewport.
